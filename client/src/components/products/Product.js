...
const Product = ({
    ...
    return (
        <div className={`w-full text-base cursor-pointer ${px && 'px-[10px]'}`}>
            <div
            ...
            >
                ...
                <div className='w-full relative'>
                    {isShowOption && <div className='absolute bottom-[-10px] left-0 right-0 flex 
                        ...
                        {current?.wishlist?.some(element => element._id === productData?._id.toString()) ?
                            <span title='Added wishlist' onClick={(e) => handleClickOptions(e, 'WISHLIST')}>
                                <SelectOption icon={<IoMdHeart color='red' />} hover='hover:bg-none' />
                            </span> :
